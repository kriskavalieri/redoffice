<template>
  <div class="form-check radio abc-radio" :class="additionalClasses">
    <input :style="{height: 0}" class="form-check-input" type="radio" :name="name" :id="id" :value="value" :checked="checkState" @change="onChange" :disabled="disabled">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toggle () {
        let emittee = this.checkState ? '' : this.value
        this.$emit('input', emittee)
      }
    },
    computed: {
      checkState () {
